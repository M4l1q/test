function drawSprite(ctx, sprite, x, y, scale = 1, options = {}) {
    ...
    if (tint) {
        const tintX = flipH || flipV ? 0 : x;
        const tintY = flipH || flipV ? 0 : y;
        ctx.globalCompositeOperation = 'source-atop';
        ctx.fillStyle = tint;
        ctx.fillRect(tintX, tintY, width, height);
...
window.drawSpriteRotated = drawSpriteRotated;
window.clearCanvas = clearCanvas;
window.setupPixelPerfectCanvas = setupPixelPerfectCanvas;
